...
function Header({ handleSubmit, spinner }) {
    return (
        <div
            className="relative flex flex-col items-center justify-center h-60 lg:h-64 bg-cover bg-center"
            style={{
                backgroundImage: `url(${window.innerWidth < 1024 ? bgMobile : bgDesktop})`,
            }}
        >
            {/* Title */}
            <h1 className="text-center text-white text-3xl lg:text-4xl font-bold p-6">
                IP Address Tracker
            </h1>

            {/* Input + Button */}
            <div className="flex mb-10 justify-center w-full px-4">
                <input
                    id="ip-input"
                    className="flex-1 max-w-xs lg:max-w-md px-4 py-3 rounded-l-2xl border-2 border-gray-300 focus:outline-none focus:ring-2 focus:ring-black"
                    placeholder="For Example: 8.8.8.8"
                />
                <button
                    onClick={() => handleSubmit("ip-input")}
                    className="bg-black text-white px-5 py-3 rounded-r-2xl hover:bg-gray-800 transition-all"
                >
                    Search
                </button>
            </div>
            ...
            <div
                role="status"
                className={`${spinner ? "block" : "hidden"} mx-auto`}
            >
                <svg
                    aria-hidden="true"
